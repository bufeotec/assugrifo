...
class Usuario{
    ...
    public function listar_persona_microtime($persona_codigo){
        try{
            $sql = 'select id_persona from personas where persona_codigo = ? limit 1';
            $stm = $this->pdo->prepare($sql);
    ...
    public function guardar_persona($model){
        ...
        try{
            if(isset($model->id_persona)){
                ...
                $sql = 'insert into personas (persona_nombre, persona_apellido_paterno, persona_apellido_materno, persona_nacimiento, persona_telefono, persona_creacion, persona_modificacion, persona_codigo) values (?,?,?,?,?,?,?,?)';
                $stm = $this->pdo->prepare($sql);
                $stm->execute([
                    $model->persona_nombre,
                    $model->persona_apellido_paterno,
