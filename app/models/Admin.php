    ...
    public function guardar_apertura_caja($model){
        ...
        try{
            if(isset($model->id_caja)){
                ...
                $sql = 'insert into caja (caja_fecha, caja_apertura,id_caja_numero, id_usuario_apertura, caja_apertura_fecha, caja_cierre, 
                        id_usuario_cierre, caja_cierre_fecha,caja_estado) values (?,?,?,?,?,?,?,?,?)';
                $stm = $this->pdo->prepare($sql);
                $stm->execute([
                    $fecha_actual,
                    $model->caja_apertura,
