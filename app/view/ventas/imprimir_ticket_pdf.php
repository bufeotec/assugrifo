...
$pdf->Cell(60,4,"CAL.ESTADO DE ISRAEL NRO. 256",0,1,'C');
$pdf->Cell(60,4,"URB. VIRGEN DE LORETO",0,1,'C');
...
//$pdf->Cell(60,4,"Fabricacion de Prendas de vestir y ventas",0,1,'C');
//$pdf->Cell(60,4,"al por menor de articulos de ferreteria.",0,1,'C');
...
foreach ($detalle_venta as $f){
    ...
    $pdf->MultiCell(60,4,"$f->venta_detalle_nombre_producto",0,'L');
    $pdf->Ln(1);
    $pdf->Cell(28, 4, "",0,0,'R');
    $pdf->Cell(7, 4, "$f->venta_detalle_cantidad",0,0,'L');
    $pdf->Cell(10, 4, number_format(round("$f->venta_detalle_precio_unitario",2), 2, ',', ' '),0,0,'R');
    $pdf->Cell(15, 4, number_format(round("$f->venta_detalle_valor_total",2), 2, ',', ' '),0,0,'R');
    $pdf->Ln(3);
}
...
$pdf->SetFont('Helvetica', '', 7);
$pdf->Multicell(60, 2, "$montoLetras", 0,'R');
// PIE DE PAGINA
$pdf->Ln(5);

if(count($detalle_venta)==1){
    //y = 136
    $pdf->Image("$ruta_qr",'8','120', '20', '20', '', '');
}elseif(count($detalle_venta)==2){
    $pdf->Image("$ruta_qr",'8','128', '20', '20', '', '');
}elseif(count($detalle_venta)==3){
    $pdf->Image("$ruta_qr",'8','136', '20', '20', '', '');
}elseif(count($detalle_venta)==4){
    $pdf->Image("$ruta_qr",'8','144', '20', '20', '', '');
}elseif(count($detalle_venta)==5){
    $pdf->Image("$ruta_qr",'8','152', '20', '20', '', '');
}elseif(count($detalle_venta)==6){
    $pdf->Image("$ruta_qr",'8','160', '20', '20', '', '');
}elseif(count($detalle_venta)==7){
    $pdf->Image("$ruta_qr",'8','166', '20', '20', '', '');
}elseif(count($detalle_venta)==8){
    $pdf->Image("$ruta_qr",'8','174', '20', '20', '', '');
}elseif(count($detalle_venta)==9){
    $pdf->Image("$ruta_qr",'8','182', '20', '20', '', '');
}elseif(count($detalle_venta)==10){
    $pdf->Image("$ruta_qr",'8','190', '20', '20', '', '');
}elseif(count($detalle_venta)==11){
    $pdf->Image("$ruta_qr",'8','198', '20', '20', '', '');
}
//$pdf->Cell(20, 10, '', 0);
